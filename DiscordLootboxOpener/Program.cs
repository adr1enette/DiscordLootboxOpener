        ...
        static async Task SendPostRequest(HttpClient client)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (jsonObject.ContainsKey("user_lootbox_data"))
                {
                    ...
                    LogMessage($"Total items: {totalItems} (rate limited: {_rateLimitedCount}, errors: {_errorCount})");
                }
                else
                {
                    LogMessage("No lootbox data found in the response.");
                }
            }
            else
            {
                ...
                if (jsonObject.ContainsKey("retry_after"))
                {
                    ...
                    _rateLimitedCount++;
                    LogMessage($"Rate limited. Waiting for {retryAfterSeconds} seconds.");
                    await Task.Delay(milliseconds);
                }
                else
                {
                    _errorCount++;
                    LogMessage($"Failed to send request. Status code: {response.StatusCode}");
                }
            }
        }
    }
